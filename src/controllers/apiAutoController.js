...
async function agregarAuto(req, res) {
  ...
  try {
    ...
    res.status(404).json({ message: error.message+ ": verifique que esten todos los datos" });
  }
}
...
async function eliminarAuto(req, res) {
  ...
  try {
    ...
    if (autoExiste === null) {
      throw new Error("No se pudo eliminar ya que NO existe el auto con el id: "+ id)
    }
...
async function modificarAuto(req, res) {
    try {
        ...
        const autoExiste = await Auto.findByPk(id);
        if (autoExiste === null) {
          throw new Error("No se pudo modificar ya que no existe el auto con el id: "+ id)
        }
